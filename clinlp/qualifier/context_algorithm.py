...
class _MatchedContextPattern:
    ...
    def __init__(self, rule: ContextRule, start: int, end: int, offset: int = 0) -> None:
        self.rule = rule
...
class ContextAlgorithm(QualifierDetector):
    ...
    @staticmethod
    def _parse_qualifier(qualifier: str, qualifier_factories: dict[str, QualifierFactory]) -> Qualifier:
        """
    ...
    def _parse_rules(self, rules: Union[str | dict]) -> list[ContextRule]:
        ...
        for qualifier in rules["qualifiers"]:
            self._qualifier_factories[qualifier["name"]] = QualifierFactory(qualifier["name"], qualifier["values"])

        qualifier_rules = []

        for rule in rules["rules"]:
            qualifier = self._parse_qualifier(rule["qualifier"], self.qualifier_factories)
